...
def trainning(loss, learning_rate):
    with tf.name_scope('optimizer'):
        ...
        train_op = optimizer.minimize(loss, global_step=global_step)#global_step=global_step 会自动加一
    return train_op
...
def evaluation(logits, labels):
    with tf.variable_scope('accuracy') as scope:
        ...
        correct = tf.cast(correct, tf.float16)# 把前面的转化为后面的类型
        accuracy = tf.reduce_mean(correct)
        tf.summary.scalar(scope.name + '/accuracy', accuracy)# 用来显示标量信息（用于收集一维标量），一般在画loss,accuary时会用到这个函数。
    return accuracy
